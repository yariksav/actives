...
class SyComponent
{
    ...
    public function __set($name,$value)
    {
        ...
        if(method_exists($this,'get'.$name))
            throw new Exception(\Yii::t('yii','Property "{class}.{property}" is read only.',
                array('class'=>get_class($this), 'property'=>$name)));
        else
            throw new Exception(\Yii::t('yii','Property "{class}.{property}" is not defined.',
                array('class'=>get_class($this), 'property'=>$name)));
    ...
    public function __call($name,$parameters)
    {
        ...
        throw new Exception(\Yii::t('yii','{class} and its behaviors do not have a method or closure named "{name}".',
    ...
    public function getEventHandlers($name)
    {
        if($this->hasEvent($name))
        {
            ...
            throw new Exception(\Yii::t('yii','Event "{class}.{event}" is not defined.',
    ...
    public function raiseEvent($name,$event)
    {
        ...
        if(isset($this->_e[$name]))
        {
            foreach($this->_e[$name] as $handler)
            {
                ...
                elseif(is_callable($handler,true))
                {
                    if(is_array($handler))
                    {
                        ...
                        elseif(method_exists($object,$method))
                            ...
                            throw new Exception(\Yii::t('yii','Event "{class}.{event}" is attached with an invalid handler "{handler}".',
                ...
                if(($event instanceof SyEvent) && $event->handled)
                    return;
            }
        }
 ...
 * @since 1.0
 */
class SyEvent extends SyComponent
{
    /**
